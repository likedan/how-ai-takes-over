...
Possible mitigations---scope enforcement, rate limiting, output review---reduce the attack surface but do not eliminate it. They are speed bumps, not walls. The strategic implication is asymmetric: invest heavily in infrastructure-level bounding for the downward gap (which is fully solvable), while accepting that the upward gap requires access control and societal adaptation rather than technical fixes.

\subsection{Research Directions for the Upward Gap}

The upward gap cannot be bounded, but it can be shaped. Four directions deserve investigation:
...
None of these fully closes the upward gap. Together, they reduce exploitability while preserving utility. The fundamental tension---that the same capability that protects also threatens---requires societal adaptation, not just technical mechanisms.
