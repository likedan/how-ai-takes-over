...
\title{Capability Without Control: Why Aligned AI Agents Cannot Govern the Consequences of Their Own Actions}
...
In both cases, the model is perfectly aligned---it follows its operator's intent faithfully and reports honestly about what it believes it has done. In both, every model-focused guardrail fails---RLHF, input classifiers, chain-of-thought monitoring---because the model is not the source of the threat. The system-side gap requires infrastructure controls. The operator-side gap requires operator accountability. Neither requires better alignment.
...
The solution operates at a different level: the \emph{operator}, not the model. The model is a tool. Like any tool, it can be used for defense or offense. The difference is not in the tool's alignment but in who wields it and whether they are authorized. Anthropic's decision to withhold Claude Mythos Preview from general release \cite{anthropic2026mythos} acknowledges this directly: the defense against the operator-side gap is access control---restricting \emph{who} can use the capability, not \emph{what} the capability can do.
...
None of these fully closes the operator-side gap. Together, they reduce exploitability while preserving utility. The fundamental tension---that the same capability that protects also threatens---requires societal adaptation to a world where the operator, not the model, is the locus of risk.
...
AI agents today have extraordinary capability and no control over consequence. This is not a bug in the models. It is a structural feature of systems where agents invoke tools whose effects exceed their understanding and serve operators whose intent they cannot verify.

We demonstrated this concretely. On the system side, a frontier model ran \texttt{pip install}, fixed a bug, reported honestly, and had no idea a cron job was created---across 7 models, 3 providers, 98.6\% of trials. On the operator side, the same model composed HTTP requests into 500 pages of exploit documentation at three orders of magnitude less cost than human expertise---identical output whether the operator defends or attacks. Both experiments used aligned models. Both evaded all guardrails. The first produced outcomes no one authorized; the second produced outcomes whose harm depends entirely on who asked.

These are different problems requiring different solutions. The system-side gap requires infrastructure-level consequence bounding---constraining what tools can actually do, regardless of intent. The operator-side gap requires shifting accountability from the model to the operator---verified identity, usage attribution, tiered capability access. Neither requires better alignment, because the model is not the source of either threat.

The gap is growing. Not because models are getting worse, but because the systems we build around them are getting more complex, more autonomous, and more connected. The tools are more powerful. The supply chains are longer. The operators are more numerous. The models are as aligned as they have ever been---and it is not enough.
