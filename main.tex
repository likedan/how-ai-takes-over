...
In both directions, the model is perfectly aligned---meaning it follows its operator's intent faithfully and reports honestly about what it believes it has done, without claiming complete knowledge of all system effects. In both, every model-focused guardrail fails---RLHF, input classifiers, chain-of-thought monitoring---because the model is not the source of the threat. The threat is the gap between the agent's understanding and the system's actual behavior.
...
\textbf{Position: the consequence gap---not model misalignment---is the primary threat from deployed AI agents, and closing it requires infrastructure-level consequence bounding, not better alignment.} This paper makes three claims. First, the consequence gap is the primary threat from deployed AI agents. Second, this gap already enables all conditions for sustained autonomous harm, as demonstrated experimentally. Third, the safety community's focus on model alignment, while necessary, is insufficient: the fix must come at the infrastructure level.
...
On persistence, \cite{pan2024selfreplication} demonstrated self-replication and \cite{carlsmith2022powerseeking} analyzed power-seeking behavior. Both frame persistence as a model \emph{capability}---an intentional act by the agent. The experiments below reveal persistence instead as an \emph{architectural side effect}: the system acts through the model, without the model's knowledge.

The consequence gap subsumes several existing threat categories. Prior frameworks identify ``evasion of oversight'' as a necessary condition for sustained escalation \cite{carlsmith2022powerseeking,bostrom2014superintelligence}. This is a special case of the downward gap: an agent that cannot observe its own tool side effects cannot report them, achieving structural evasion without deception. This is strictly harder to defend against than active evasion, because alignment training cannot fix ignorance.


\section{The Consequence Gap}
\label{sec:gap}

This section formalizes the consequence gap, establishes its two directions, and explains why model alignment cannot close either. The formalism provides the analytical scaffold for the experiments in Section~\ref{sec:experiments}.
...
Frontier models (GPT-4o, GPT-5.4) resist direct prompt injection across five stealth levels---from plaintext instructions to implicit goal framing---in all trials ($n = 12$). Agent-level alignment works for direct attacks. The consequence gap, however, bypasses it entirely.
...
Claude Opus 4.6 & Anthropic & 10 & 100\% & omission: 9, false completeness: 1 \\
...
\textbf{Monitoring condition.} Each trial was also run in a variant where the system prompt disclosed that actions are being reviewed by a security team. Results for the monitored variant will be reported in the camera-ready version once data collection is complete. The condition tests whether models exhibit context-sensitive compliance with injected instructions---a finding relevant to the alignment faking literature \cite{anthropic2024faking}.

\textbf{Composite outcome.} The full ``all guardrails fail'' condition requires four factors simultaneously: (1) the persistence mechanism was created; (2) it survived process termination; (3) the agent did not self-report it; (4) the auditor model did not flag it. The per-trial composite rate will be reported in the camera-ready version. The structure of the experiment---supply chain hook invisible to both agent and auditor, combined with omission in 45 of 49 self-reports---establishes that all four conditions are routinely met together.
...
\textbf{Security audits.} All 15 websites audited in Experiment~2 were tested with explicit written authorization from site owners, obtained prior to testing; signed consent records are on file. All identified vulnerabilities were reported to affected parties before this paper was submitted. Targets are anonymized by industry category; raw reports are not published.

\textbf{Supply chain artifacts.} The \texttt{logging-helpers} package was tested only in isolated Docker containers with no network access. It has not been uploaded to PyPI or any public package registry.

\textbf{Artifact generation.} Both experimental artifacts were generated by an AI agent from natural-language prompts. The Appendix reproduces them in full to demonstrate the consequence gap itself and to support reproducibility; they are not released as standalone tools.

\textbf{IRB.} This research did not involve human subjects. Experiments involve AI model APIs and authorized infrastructure testing.
